...
class Cosmosfarm_Members_Subscription_Order_Table extends WP_List_Table {
	...
	public function single_row($order){
		...
		echo '<td class="status-'.$order->status().'" data-colname="결제 상태">';
		echo $order->status_format();
		echo '</td>';
		
		echo '<td class="subscription-next-'.$order->subscription_next().'" data-colname="정기결제 상태">';
		...
		echo '<td data-colname="다음 정기결제 실패">';
		echo $order->error_message();
		echo '</td>';
		
		echo '<td data-colname="정기결제 회차">';
		echo sprintf('%s회차', $order->pay_count());
		echo '</td>';
		
		if($user && $user->ID){
			echo '<td data-colname="사용자">';
		...
		/*
		echo '<td data-colname="주문자명">';
		echo $order->buyer_name;
		echo '</td>';
		
		echo '<td data-colname="이메일">';
		echo $order->buyer_email;
		echo '</td>';
		
		echo '<td data-colname="전화번호">';
