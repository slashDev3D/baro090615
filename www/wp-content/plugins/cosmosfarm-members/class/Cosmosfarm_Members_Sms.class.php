...
class Cosmosfarm_Members_Sms {
	...
	public function send_every($phone_field, $content){
		$option = get_cosmosfarm_members_option();
		if(!$this->is_active()){
			return array('result'=>'error', 'message'=>'SMS 보내기 설정이 사용중지 되었습니다.');
		}
		...
		if($option->sms_service == 'cafe24'){
			...
			return $this->send_execute_cafe24($phone_list, $content);
		}
		else if($option->sms_service == 'toast_cloud'){
			$phone_list = array();
			foreach($user_query->get_results() as $user){
			...
			return $this->send_execute_toast_cloud($phone_list, $content);
		}
	}
